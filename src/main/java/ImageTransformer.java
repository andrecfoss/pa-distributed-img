...
public class ImageTransformer {
    ...
    /**
     * Remove the red component from the given image.
     *
     * @param image The image from which to remove the red component.
     ...
     * @return A BufferedImage with the red component removed.
     */
    public static BufferedImage removeRedFullImage (BufferedImage image , int nRows , int nCols,LoadInfoReader loadInfoReader, LoadInfoWriter loadInfoWriter) {
        SIMDExecutor simdExecutor = new SIMDExecutor ( image , nRows , nCols , image.getWidth ( ) , image.getHeight ( ) , image.getType ( ),loadInfoReader, loadInfoWriter);
        simdExecutor.execute ( );
        return simdExecutor.getImageWithoutRed ( );
    }
}